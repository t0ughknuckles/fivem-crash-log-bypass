...
{
    DWORD oldProtect;
    if (!VirtualProtect(function, 1, PAGE_EXECUTE_READWRITE, &oldProtect)) { return 1; }

    DetourTransactionBegin();
    DetourUpdateThread(GetCurrentThread());

    if (DetourAttach(&(PVOID&)function, Hook) != NO_ERROR) { DetourTransactionAbort(); return 1; }
    if (DetourTransactionCommit() != NO_ERROR) { return 1; }
